...
module mnist_tb;
    ...
    task automatic wait_gemm_compute(input int a_dim, input int w_dim, input int k_dim, input logic tile_last_i);
        begin
            ...
            @(posedge clk); // wait 1 extra cycle for state transition to GEMM_FLUSH
    ...
    initial begin
        ...
        for (int set_y = 0; set_y < 32; set_y++) begin 
            ...
            multi_tile_test(1, 8, 8, 98, 1, set_y, 1'b1); // enable relu for the first layer
        end
        ...
        for (int set_y = 0; set_y < 16; set_y++) begin 
            multi_tile_test(1, 8, 8, 32, 2, set_y, 1'b1); // enable relu for the second layer
        end
